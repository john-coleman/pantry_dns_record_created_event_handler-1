...
describe Wonga::Daemon::DnsRecordCreatedEventHandler do
  ...
  it "updates Pantry using PantryApiClient" do
    expect(api_client).to receive(:send_put_request).with("/api/ec2_instances/40", { "user_id" => 1,  "pantry_request_id" => 40, "instance_id" => "i-0123abcd", :event => :create_dns_record, :joined => true, :dns => true })
